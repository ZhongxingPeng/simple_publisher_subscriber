...
void* ModuleA::run() {
  for (int i = 0;; i++) {
    ...
    // Uncomment the following block to test the in-class subscriber/publisher
    // We use getServerHandle() to obtain the handle for the message server
    ////////////////////////////////////////////////////////////////////////////

    // if (temp_msg_server_handle != NULL && is_message_server_exist == false) {
    //   // Change the switch
    ...
    // Once receive any message, print the message, then publish a new message
    // "A_welcome_from_ModuleA" to a topic called "Topic for moduleB", which is
    // subscribed by thread of ModuleB
    if (!isMssageBufferEmpty()) {
      // Fetch a msg from message-buffer, then remove the msg from the buffer
...
void* ModuleB::run() {
  for (int i = 0;; i++) {
    ...
    // Once receive any message, print the message, then publish a new message
    // "A_welcome_from_ModuleB" to a topic called "Topic for moduleA", which is
